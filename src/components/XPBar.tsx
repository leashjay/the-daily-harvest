...
export default function XPBar({ player }: XPBarProps) {
  ...
  return (
    <div className="flex items-center gap-4 bg-white/80 backdrop-blur rounded-2xl px-5 py-3 shadow-sm border border-garden-light">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between text-sm mb-1">
          <span className="font-heading font-semibold text-garden-green">{player.name} — {name}</span>
          <span className="text-garden-soil text-xs">
            {player.xp} / {next ?? '∞'} XP
          </span>
        </div>
